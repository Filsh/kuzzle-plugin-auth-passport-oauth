...
module.exports = function (context, config) {
  ...
  this.verify = (accessToken, refreshToken, profile, done) => {
    ...
    if (this.config.strategies[profile.provider].persist) {
      user = this.createUserObject(profile, this.config.strategies[profile.provider].persist);
    }
  ...
  this.createUserObject = (profile, persist) => {
    var
      user = {},
      _get = (obj, path) => path.split('.').reduce((obj, i) => !obj ? null : obj[i], obj),
      _set = (obj, path, value) => {
      ...
      Object.keys(persist).forEach(attr => {
        if (typeof persist[attr] === 'string') {
          _set(user, attr, _get(profile._json, persist[attr]));
        } else {
          _set(user, attr, persist[attr]);
        }
      });
